...
from models.mobilenet_v1_extended import mobilenet
...
def ssd_300(mode,
    ...
    conv3 , conv4_3_norm , fc7= mobilenet(input_tensor=x1)
    ...
    conv3_norm = L2Normalization(gamma_init=20, name='conv3_norm')(conv3)
   
    conv3_norm_mbox_conf = Conv2D(n_boxes[0] * n_classes, (1,1), padding='same', kernel_initializer='he_normal',
                                    kernel_regularizer=l2(l2_reg), name='conv3_mbox_conf')(conv3_norm)
    conv4_3_norm_mbox_conf = Conv2D(n_boxes[1] * n_classes, (1,1), padding='same', kernel_initializer='he_normal',
                                    kernel_regularizer=l2(l2_reg), name='conv11_mbox_conf')(conv4_3_norm)
    fc7_mbox_conf = Conv2D(n_boxes[2] * n_classes, (1,1), padding='same', kernel_initializer='he_normal',
                           kernel_regularizer=l2(l2_reg), name='conv13_mbox_conf')(fc7)
    conv6_2_mbox_conf = Conv2D(n_boxes[3] * n_classes, (1,1), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv14_2_mbox_conf')(conv6_2)
    conv7_2_mbox_conf = Conv2D(n_boxes[4] * n_classes, (1,1), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv15_2_mbox_conf')(conv7_2)
    conv8_2_mbox_conf = Conv2D(n_boxes[5] * n_classes, (1,1), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv16_2_mbox_conf')(conv8_2)
    conv9_2_mbox_conf = Conv2D(n_boxes[6] * n_classes, (1,1), padding='same', kernel_initializer='he_normal',
                               kernel_regularizer=l2(l2_reg), name='conv17_2_mbox_conf')(conv9_2)
    # We predict 4 box coordinates for each box, hence the localization predictors have depth `n_boxes * 4`
    # Output shape of the localization layers: `(batch, height, width, n_boxes * 4)`


    conv3_norm_mbox_loc = Conv2D(n_boxes[0] * 4, (1,1), padding='same', kernel_initializer='he_normal',
                                   kernel_regularizer=l2(l2_reg), name='conv3_mbox_loc')(conv3_norm)
    conv4_3_norm_mbox_loc = Conv2D(n_boxes[1] * 4, (1,1), padding='same', kernel_initializer='he_normal',
                                   kernel_regularizer=l2(l2_reg), name='conv11_mbox_loc')(conv4_3_norm)
    fc7_mbox_loc = Conv2D(n_boxes[2] * 4, (1,1), padding='same', kernel_initializer='he_normal',
                          kernel_regularizer=l2(l2_reg), name='conv13_mbox_loc')(fc7)
    conv6_2_mbox_loc = Conv2D(n_boxes[3] * 4, (1,1), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv14_2_mbox_loc')(conv6_2)
    conv7_2_mbox_loc = Conv2D(n_boxes[4] * 4, (1,1), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv15_2_mbox_loc')(conv7_2)
    conv8_2_mbox_loc = Conv2D(n_boxes[5] * 4, (1,1), padding='same', kernel_initializer='he_normal',
                              kernel_regularizer=l2(l2_reg), name='conv16_2_mbox_loc')(conv8_2)
    conv9_2_mbox_loc = Conv2D(n_boxes[6] * 4, (1,1), padding='same', kernel_initializer='he_normal',
    ...
    conv3_norm_mbox_priorbox = AnchorBoxes(img_height, img_width, this_scale=scales[0], next_scale=scales[1],
                                             ...
                                             name='conv3_norm_mbox_priorbox')(conv3_norm_mbox_loc)
    conv4_3_norm_mbox_priorbox = AnchorBoxes(img_height, img_width, this_scale=scales[1], next_scale=scales[2],
                                             aspect_ratios=aspect_ratios[1],
                                             two_boxes_for_ar1=two_boxes_for_ar1, this_steps=steps[1],
                                             this_offsets=offsets[1], limit_boxes=limit_boxes,
                                             variances=variances, coords=coords, normalize_coords=normalize_coords,
                                             name='conv4_3_norm_mbox_priorbox')(conv4_3_norm_mbox_loc)
    fc7_mbox_priorbox = AnchorBoxes(img_height, img_width, this_scale=scales[2], next_scale=scales[3],
                                    aspect_ratios=aspect_ratios[2],
                                    two_boxes_for_ar1=two_boxes_for_ar1, this_steps=steps[2], this_offsets=offsets[2],
                                    limit_boxes=limit_boxes,
                                    variances=variances, coords=coords, normalize_coords=normalize_coords,
                                    name='fc7_mbox_priorbox')(fc7_mbox_loc)
    conv6_2_mbox_priorbox = AnchorBoxes(img_height, img_width, this_scale=scales[3], next_scale=scales[4],
    ...
    conv7_2_mbox_priorbox = AnchorBoxes(img_height, img_width, this_scale=scales[4], next_scale=scales[5],
    ...
    conv8_2_mbox_priorbox = AnchorBoxes(img_height, img_width, this_scale=scales[5], next_scale=scales[6],
    ...
    conv9_2_mbox_priorbox = AnchorBoxes(img_height, img_width, this_scale=scales[6], next_scale=scales[7],
                                        aspect_ratios=aspect_ratios[6],
                                        two_boxes_for_ar1=two_boxes_for_ar1, this_steps=steps[6],
                                        this_offsets=offsets[6], limit_boxes=limit_boxes,
                                        variances=variances, coords=coords, normalize_coords=normalize_coords,
    ...
    conv3_norm_mbox_conf_reshape = Reshape((-1, n_classes), name='conv3_norm_mbox_conf_reshape')(
        conv3_norm_mbox_conf)
    ...
    conv3_norm_mbox_loc_reshape = Reshape((-1, 4), name='conv3_norm_mbox_loc_reshape')(conv3_norm_mbox_loc)
    ...
    conv3_norm_mbox_priorbox_reshape = Reshape((-1, 8), name='conv3_norm_mbox_priorbox_reshape')(
        conv3_norm_mbox_priorbox)
    ...
    mbox_conf = Concatenate(axis=1, name='mbox_conf')([conv3_norm_mbox_conf_reshape,
    ...
    mbox_loc = Concatenate(axis=1, name='mbox_loc')([conv3_norm_mbox_loc_reshape,
    ...
    mbox_priorbox = Concatenate(axis=1, name='mbox_priorbox')([conv3_norm_mbox_priorbox_reshape,
